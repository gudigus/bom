			...
			<div class="container-fluid">
				...
				<div class="card">
					<div class="card-body">
						...
						<a class="card-title text-dark">${board.unickName }</a> <a
				...
				<c:forEach var="reply" items="${replylist }" varStatus="status">
					...
					<!-- 경빈 part2 -->
						<div class="card-body" id="singleBoard">
							...
							<img src="<%=context %>/profile_image/${reply.uimage }"
								alt="no_image" class="rounded-circle" width="50" width="50">
							<a class="card-title text-dark">${reply.unickName }</a> <a
								class="card-subtitle mb-2 text-muted">@${reply.uatid }</a> <a
							...
							<c:if test="${reply.battach!=null }">
								<c:if test="${reply.battachType=='image'}">
									<img class="img-thumnail" width="300"
										src="<%=context %>/image/${reply.battachSrc}" />
								</c:if>
								<c:if test="${reply.battachType=='video'}">
									<video controls width="300">
										<source src="<%=context %>/video/${reply.battachSrc}"
											type="video/mp4">
										<source src="<%=context %>/video/${reply.battachSrc}"
											type="video/webm">
										해당 브라우저에는 지원하지 않는 비디오입니다.
